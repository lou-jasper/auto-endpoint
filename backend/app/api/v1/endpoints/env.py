...
from app.adapters.db.repositories.env_repo import (
    EnvironmentRepository,
    EnvironmentHeaderRepository,
    VariableExtractionRuleRepository
)
...
from app.api.v1.schemas.env_schema import (
    ...
    EnvironmentCreate,
    EnvironmentUpdate,
    EnvironmentHeaderRead,
    EnvironmentHeaderCreate,
    GlobalVariableRead,
    GlobalVariableCreate,
    VariableExtractionRuleRead,
    VariableExtractionRuleCreate
)

from app.core.services.env_service import (
    EnvironmentService,
    EnvironmentHeaderService,
    VariableExtractionRuleService
)
...
# --------------------------------------------------
# Service Providers (统一依赖注入)
# --------------------------------------------------

def get_environment_service(
        session: AsyncSession = Depends(get_db_session)
) -> EnvironmentService:
    return EnvironmentService(EnvironmentRepository(db=session))


def get_environment_header_service(
        session: AsyncSession = Depends(get_db_session)
) -> EnvironmentHeaderService:
    return EnvironmentHeaderService(EnvironmentHeaderRepository(db=session))


def get_variable_extraction_rule_service(
        session: AsyncSession = Depends(get_db_session)
) -> VariableExtractionRuleService:
    return VariableExtractionRuleService(VariableExtractionRuleRepository(db=session))
...
async def list_headers(
    ...
    return await service.list_headers(env_id)
...
async def create_global_variable(
    ...
    return await service.create_global_variable(data)
...
async def create_extraction_rule(
        ...
        service: VariableExtractionRuleService = Depends(get_variable_extraction_rule_service)
):
    return await service.create_rule(var_id, data)
...
async def get_extraction_rule(
    ...
    rule = await service.get_rule(var_id)
    if not rule:
